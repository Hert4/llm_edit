    ...
    for request in requests:
        ...
        with torch.no_grad():
            for w_name, (delta_u, delta_v) in deltas.items():
                ...
                # Handle MoE expert weights (3D tensor: num_experts x hidden x intermediate)
                if hparams.is_moe and len(w.shape) == 3:
                    # Apply update to all experts
                    upd_matrix_2d = upd_matrix_match_shape(upd_matrix, w.shape[1:])
                    for expert_idx in range(w.shape[0]):
                        w[expert_idx, ...] += upd_matrix_2d
                    print(f"Updated {w.shape[0]} experts in {w_name}")
                else:
...
def execute_rome(
    ...
    # For MoE models, experts.down_proj is a Parameter directly (no .weight suffix)
    # For dense models, down_proj is a Linear module (needs .weight suffix)
    def get_weight_name(layer):
        if hparams.is_moe:
            return hparams.rewrite_module_tmp.format(layer)
        else:
            return f"{hparams.rewrite_module_tmp.format(layer)}.weight"

    weights = {get_weight_name(layer): nethook.get_parameter(model, get_weight_name(layer))
    ...
    for layer in sorted(hparams.layers):
        ...
        with torch.no_grad():
            ...
            weight_name = get_weight_name(layer)
            upd_matrix = left_vector.unsqueeze(1) @ right_vector.unsqueeze(0)

            # Handle MoE expert weights (3D tensor)
            if hparams.is_moe and len(weights[weight_name].shape) == 3:
                upd_matrix_2d = upd_matrix_match_shape(upd_matrix, weights[weight_name].shape[1:])
                for expert_idx in range(weights[weight_name].shape[0]):
                    weights[weight_name][expert_idx, ...] += upd_matrix_2d
            else:
